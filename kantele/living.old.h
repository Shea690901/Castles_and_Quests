 ...
 * Addition by some stuff from the current living.c /Pell
 */
hit_player(dam) {
    set_heart_beat(1);		/* Pell */
    if (!attacker_ob && this_player() != this_object())	/* Pell */
	attacker_ob = this_player(); /* Pell */
    else if (!alt_attacker_ob && attacker_ob != this_player() && /* Pell */
	     this_player() != this_object()) /* Pell */
	alt_attacker_ob = this_player(); /* Pell */
